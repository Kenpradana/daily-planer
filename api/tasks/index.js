  ...
  try {
    ...
    if (req.method === 'GET' && req.query.view === 'stats') {
      ...
      const { count: done, error: e2 } = await supabase
        .from('tasks').select('*', { count: 'exact', head: true }).eq('done', true);
      if (e2) throw e2;
    ...
    if (req.method === 'POST') {
      ...
      if (!text || !date_key) return res.status(400).json({ error: 'text dan date_key wajib' });
      const { data, error } = await supabase
        .from('tasks')
        .insert([{ date_key, text: text.trim(), time: time || '', note: note || '', category: category || 'other', done: false }])
        .select().single();
      if (error) throw error;
